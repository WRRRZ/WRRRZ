export const iii6_stage = `
<div id='video' class='boxpost1'>
<video width="426" height="320" autoplay>
  <source src="./images/clip1.mp4" type="video/mp4">
Your browser does not support the video tag.
</video>

</div>
<div id='text' class='boxpost2'>
    <div id='' class='txt'>WRRRZ is a competitive open-world platformer game set in a stylistic, futuristic African village named Kente. The game blends unique fashion elements with combat and strategy, centered around clan-based looting mechanics to acquire valuable fashion items that enhance player abilities.</div>
</div>`;
export const design_stage = `
<div id='desdyn' class='boxpost1'>
    <h1 id='' class='title'>Codebender Dynamic NFTs Editor</h1>
    <div id='' class='txt'></div>
    <img src='' class='img' />
</div>
<div id='desdapps' class='boxpost2'>
    <h1 id='' class='title'>Interactive dApp SDK</h1>
    <div id='' class='txt'></div>
    <img src='' class='img' />
</div>`;
export const develop_stage = `
<div id="coinbuild" class="barpost">
  <h1 id="" class="title">Create a custom ERC20 Token Contract</h1>
  <div id='' class='txt'></div>
  <img src='' class="img" />
</div>
<div id="nftbuild" class="barpost">
  <h1 id="" class="title">Create a custom dynamic NFT Contract</h1>
  <div id='' class='txt'></div>
  <img src='' class="img" />
</div>
<div id="sftbuild" class="barpost">
  <h1 id="" class="title">Create a custom dynamic SFT Contract</h1>
  <div id='' class='txt'></div>
  <img src='' class="img" />
</div>`;
export const launch_stage = `<div id='showbody'>launch</div>`;
export const info_stage = `<div id='showbody'>info</div>`;
